...
class Transfer(object):
    ...
    def train(self, epochs, data, noise = True, tags = None, batch_size = 32, print_interval = 5, save_interval = 1):
        '''
        ...
        for epoch in range(epochs):
            ...
            if noise:
                train_data = add_noise(data_read_ = train_data, mean = 1e-2, std = 1e-2)
          #loss_d = np.zeros(int(train_data.shape[0]/batch_size), dtype = np.float32)
          #acc_d = np.zeros(int(train_data.shape[0]/batch_size), dtype = np.float32)
          #combine_d = np.zeros(int(train_data.shape[0]/batch_size), dtype = np.float32)
            ...
            if epoch % save_interval == 0 or epoch == epochs - 1:
                self.save_and_plot_images(epoch, tags = tags)
